...
| File | Purpose |
| --- | --- |
| `plink_cnv_output.tsv` | Safe converted CNV rows only, in PLINK-style format. |
| `review_flags.tsv` | Every input row/event that was emitted, skipped, or flagged, with a reason. |
| `optional_debug_extracted_events.tsv` | Event-level debug table before final conversion. |
